...
class Interactor {
  ...
  _upHandler(event) {
    ...
    if (this.firstClick) {
      this.pressingTimeout = setTimeout(() => {
        if (this.coordsOnDown && this.coordsOnDown.x === eventClientX(event) && this.coordsOnDown.y === eventClientY(event)) {
  ...
  _touchMoveHandler(event) {
    ...
    if (event.targetTouches.length === 2) {
      ...
      // Math.hypot() analog
      const fingersHypotNew = Math.round(Math.sqrt(Math.pow(Math.abs(pageX1 - pageX2), 2) + Math.pow(Math.abs(pageY1 - pageY2), 2)))
